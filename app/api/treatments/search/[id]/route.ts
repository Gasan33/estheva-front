...
export async function GET(req: NextRequest) {
    ...
    try {
        ...
        const response = await fetch(`${config.env.apiEndpoint}/treatments/search/${id}`, {
            cache: "no-store",
            headers: {
        ...
        if (!response.ok) {
            return NextResponse.json({ error: "Failed to fetch treatments" }, { status: response.status });
        }
